...
type ProductSummary = { id: string; name: string; slug: string; price_egp: number; stock_quantity: number; display_order: number; is_active: boolean };
...
export default function AdminDashboard() {
  ...
  async function addProduct(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      collection: form.get('collection'), stockMessage: form.get('stockMessage'), stockQuantity: Number(form.get('stockQuantity')), displayOrder: Number(form.get('displayOrder')),
  ...
  async function updateStock(product: ProductSummary) {
    if (!accessToken) return;
    const stockQuantity = Number(stockDrafts[product.id] ?? product.stock_quantity);
    if (!Number.isInteger(stockQuantity) || stockQuantity < 0) {
      setNotice('Stock quantity must be a whole number of zero or more.');
      return;
    }
    ...
    if (!response.ok) {
      setNotice(result?.error ?? 'Unable to update stock.');
      return;
    }

    setProducts((current) => current.map((item) => item.id === product.id ? { ...item, stock_quantity: result.product.stock_quantity } : item));
    setStockDrafts((current) => ({ ...current, [product.id]: String(result.product.stock_quantity) }));
